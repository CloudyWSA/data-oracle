...
export default function FileUploader({ onDataLoaded, setIsLoading, isLoading }: FileUploaderProps) {
  ...
  const processCSV = useCallback(
    (file: File) => {
      ...
      reader.onload = (e) => {
        ...
        const likelyDelimiter = counts.reduce(
        ...
        const parseResult = Papa.parse(content, {
          ...
          complete: (results) => {
            ...
            const hasEssentialIds = parsedData.some(
              (row) => row && typeof row === "object" && row.gameid && row.participantid,
            )
            if (!hasEssentialIds) {
        ...
        .then((XLSX) => {
          ...
          reader.onload = (e) => {
            ...
            try {
              ...
              addDebugInfo(
                `Excel parsed. Found ${workbook.SheetNames.length} sheets: ${workbook.SheetNames.join(", ")}`,
              )
              ...
              workbook.SheetNames.forEach((sheetName) => {
                ...
                if (jsonData && jsonData.length > 0) {
                  ...
                  if (jsonData.length > 0) {
                    ...
                    if (!columnsDetected) {
                      // Store columns from first sheet with data
              ...
              // **Minimal Validation: Check if essential IDs exist in at least one row**
              const hasEssentialIds = allData.some(
                (row) => row && typeof row === "object" && row.gameid && row.participantid,
              )
              if (!hasEssentialIds) {
                setError(
                  "Could not find essential 'gameid' and 'participantid' in any row. Check file structure/headers.",
                )
                setIsLoading(false)
                addDebugInfo("Error: Essential columns missing from all rows.")
                return
              ...
              setProgress(100)
              addDebugInfo(
                `Excel processing complete. Total rows extracted: ${allData.length}. Preparing to call onDataLoaded.`,
              )
  ...
  const processFile = useCallback(
    (file: File) => {
      ...
      const isExcel =
        /\.(xlsx|xls)$/i.test(file.name) || file.type.includes("spreadsheetml") || file.type.includes("ms-excel")
      const isCsv = /\.(csv)$/i.test(file.name) || file.type.includes("csv")

      addDebugInfo(`File type detection: Excel=${isExcel}, CSV=${isCsv}, Name=${file.name}, MIME=${file.type}`)

      if (isCsv) {
        // Prioritize CSV if extension is .csv
        processCSV(file)
      } else if (isExcel) {
        processExcel(file)
      } else {
      ...
      try {
        const results = Papa.parse(content, {
          ...
          complete: (res) => {
            ...
            if (data.length > 0 && res.meta.fields && res.meta.fields.length > 1) {
              // Check for >1 column as sign of success
              addDebugInfo(
                `   Success with delimiter '${delimiter === "\t" ? "\\t" : delimiter}'! Found ${data.length} rows.`,
              )
              console.log("[UPLOADER ALT PARSE ROW 1]", data[0])
              if (data.length > 11) console.log("[UPLOADER ALT PARSE ROW 12]", data[11])

              const hasEssentialIds = data.some(
                (row) => row && typeof row === "object" && row.gameid && row.participantid,
              )
              if (hasEssentialIds) {
              ...
              addDebugInfo(
                `   Delimiter '${delimiter === "\t" ? "\\t" : delimiter}' did not yield sufficient columns or rows.`,
              )
            }
          },
          error: (err) => {
        ...
        addDebugInfo(
          `   Exception during parse attempt with '${delimiter === "\t" ? "\\t" : delimiter}': ${e instanceof Error ? e.message : "Unknown error"}`,
        )
      }
    } // End for loop
  ...
  const loadSampleData = useCallback(() => {
    ...
    for (let i = 1; i <= 5; i++) {
      sampleData.push({
        ...
        playername: `Vasco Player ${i}`,
        teamname: "Vasco da Gama",
        champion: blueChamps[i - 1],
        result: 1,
        kills: Math.floor(Math.random() * 8),
        deaths: Math.floor(Math.random() * 5),
        assists: Math.floor(Math.random() * 10),
        damageshare: (20 + Math.floor(Math.random() * 10)) / 100,
        earnedgoldshare: (20 + Math.floor(Math.random() * 10)) / 100,
        cspm: 7 + Math.random() * 3,
        vspm: 1 + Math.random() * 1.5,
        dpm: 400 + Math.floor(Math.random() * 300),
      })
    }
    for (let i = 6; i <= 10; i++) {
      sampleData.push({
        ...
        playername: `Flamengo Player ${i - 5}`,
        teamname: "Flamengo",
        champion: redChamps[i - 6],
        result: 0,
        ...
        damageshare: (20 + Math.floor(Math.random() * 10)) / 100,
        earnedgoldshare: (20 + Math.floor(Math.random() * 10)) / 100,
        cspm: 6 + Math.random() * 3,
        vspm: 1 + Math.random() * 1.5,
        dpm: 350 + Math.floor(Math.random() * 300),
      })
    }
              ...
              <AccordionContent>
                <div className="text-sm text-vasco-gray space-y-2">
                  ...
                  <ul className="list-disc pl-5 space-y-1">
                    <li>
                      <strong>gameid</strong>: Unique ID for the game.
                    </li>
                    <li>
                      <strong>participantid</strong>: 1-10 for players, 100 for Blue team, 200 for Red team.
                    </li>
                    <li>
                      <strong>side</strong>: "Blue" or "Red".
                    </li>
                    <li>
                      <strong>position</strong>: "top", "jng", "mid", "bot", "sup" (for players), "team" (for teams).
                    </li>
                    <li>
                      <strong>teamname</strong>: Name of the team (present on both player and team rows).
                    </li>
                    <li>
                      <strong>playername</strong>: Player's name (only on player rows).
                    </li>
                    <li>
                      <strong>champion</strong>: Champion name (only on player rows).
                    </li>
                    <li>
                      <strong>result</strong>: 1 for win, 0 for loss.
                    </li>
                    <li>
                      <strong>pick1, pick2, pick3, pick4, pick5</strong>: Champion names in team pick order (***only on
                      team rows*** - participantid 100/200).
                    </li>
                    <li>
                      <em>Other stats...</em> (kills, deaths, assists, damageshare, etc. - mainly relevant on player
                      rows, but might exist on team rows as totals).
                    </li>
                  </ul>
                  <p>
                    <strong>Crucially:</strong> Ensure the team rows (participantid 100 and 200) contain the `pick1`
                    through `pick5` columns accurately.
                  </p>
                </div>
              </AccordionContent>
            </AccordionItem>
